$(document).ready(function () {
    if(isIV){
        if(df = callParentNew(`dfval${fldname}`)){
            try {
                $(".adv-dv1").show();
            // $("#ddlCondition").val("=").prop("disabled", true);
            // $("#ddlSearchFld").empty().append(df.split("|").map(dd=>`<option value="${dd.split("~")[0]}">${dd.split("~")[1] || dd.split("~")[0]}</option>`).join(""));

            /*
            df.split("|").map(dd=>`<option value="${dd.split("~")[0]}">${dd.split("~")[1] || dd.split("~")[0]}</option>`).join("")
            df.split('|').Select(dd=>`<option value="${dd.split("~")[0]}">${dd.split("~")[1] || dd.split("~")[0]}</option>`).join("")
            */
            } catch (ex) {}
        }else{
            $(".adv-dv1").hide();
