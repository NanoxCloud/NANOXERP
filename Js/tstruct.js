            ...
            try {
                CKEDITOR.instances[$(currentElm).attr("id")].on("instanceReady", function () {
                    if(!staticRunMode) {
                        ...
                        this.resize('100%', $(this.ui.editor.container.$).height() -  $(this.ui.contentsElement.$).siblings().toArray().reduce((total, elm)=>{
...
function MainBlur(fldObj) {
    ...
    if (AxDoBlur == true) {
        ...
        if (!errorFlag) {
            if (!AxRulesDefParser(fldId, "field", "validation")) {
                setTimeout(function () {
                    fldObj.focus();
                }, 0);
                errorFlag = true;
        ...
        if (AxExecFormControl && AxFldFormControl == "" && isOverridFomrControl) {
            DoFormControl(fldId);
            ...
            if ((depFirstChar == 's' || depFirstChar == "f") && isSkipSqlFillFields == false) {
                ...
                if (callService) {
                    ...
                    for (var aedi = 0; aedi < depArray.length; aedi++) {
                        ...
                        if (FMoe[aedfldInd].toString().toLowerCase() == "accept" && FldIsSql[aedfldInd].toString().toLowerCase() != "true" && aedepFirstChar == "e" )
...
function ValidateBeforeSubmit(valDcNo) {
    ...
    for (var i = 0; i < FNames.length; i++) {
        ...
        if(AxRulesDefAllowEmpty=="true" && !AxRulesDefParser(FNames[i],"field","allowempty"))
            return false;
...
function AxRulesDefParser(thisEleName,thisEleType,thisEvent,thisExpDep="")
{
    ...
    if(AxRDCompName.indexOf(flname)>-1)
    {
        let comInd=AxRDCompName.indexOf(flname);
        let comType=AxRDCompType[comInd];        

        switch (thisEvent) {
            case 'validation':
                var flval="";
                if(comType.toLowerCase() =="field")
                   flval=GetFieldValue(thisEleName);
                let vexpr=AxRDValidation[comInd];                    
                if (vexpr != "" && (flval && (flval != "" || flval != dateString)) && AxRuleIsApplicable(flname,comInd)) {
                    var fResult = Evaluate(flname, flval, vexpr, "vexpr");
                    if (fResult != 'T' && fResult != 't' && fResult != true) {
                        var cutMsg = eval(callParent('lcm[52]'));
                        var firstChar = fResult.substring(0, 1);
                ...
                break;
            case 'filter':
                let axrFilter=AxRDFilter[comInd];
                if(axrFilter!="" && AxRuleIsApplicable(flname,comInd))
                    return axrFilter;
                else
                    return "";
                break;
            case 'formcontrol':
                let axralFc=AxRDFormControl[comInd];
                if(axralFc!="" && AxRuleIsApplicable(flname,comInd)){
                EvaluateScriptFormControl(axralFc,flname);
                return false;
                }else
                return true;
                break;
            case 'computescript':
                let axrExp=AxRDComputeScript[comInd];
                if(axrExp!="" && AxRuleIsApplicable(flname,comInd)){
                    let thisDcNo=GetDcNo(flname);
                    if(IsGridField(flname)){
                        if(thisExpDep!=""){
                            let chnDcNo= thisExpDep.substring(thisExpDep.lastIndexOf("F")+1);
                            if(chnDcNo!=thisDcNo){
                                var rowCnt = 0;
                                rowCnt = parseInt(GetDcRowCount(thisDcNo), 10);
                                for (var i = 1; i <= rowCnt; i++) {
                                    var rowNo = GetClientRowNo(i, thisDcNo);
                                    AxActiveRowNo = GetDbRowNo(rowNo, thisDcNo);
                                    var tmpFldId = flname + rowNo + "F" + thisDcNo;
                                    result = Evaluate(tmpFldId, rowNo, expression, "expr");
                                    if ( (result != undefined) || (result.toString() != "undefined")) {
                                        Assignfldval(tmpFldId, result);
                                    }
                                }
                            }else{
                                let thisEleRowNo=thisEleName.substring(thisEleName.lastIndexOf("F"),thisEleName.lastIndexOf("F")-3);
                                let exprFldName = flname +thisEleRowNo+ "F" + thisDcNo;
                                let result =Evaluate(exprFldName, thisEleRowNo, axrExp, "expr");
                                if (result != undefined || result.toString() != "undefined") {
                        ...
                        let exprFldName = flname + "000F" + thisDcNo;
                        let result =Evaluate(exprFldName, "000", axrExp, "expr");
                        if (result != undefined || result.toString() != "undefined") {
                            Assignfldval(exprFldName, result);
                        }
                    }
            ...
            case 'allowduplicate': 
                ...
                break;
            case 'allowempty':
                let axralEmpty=AxRDAllowEmpty[comInd];
                if(axralEmpty!="" && axralEmpty.toLowerCase()=="f" && AxRuleIsApplicable(flname,comInd)){
...
function AxFilterDropDownResult(thisFld,result)
{
    ...
    if (axrFilter != "") {    
        if(axrFilter.toLowerCase().indexOf("not in(")>-1)
        {
            let fliterVals=axrFilter.substr(7);
            fliterVals=fliterVals.substr(0,fliterVals.length-1);
            fliterVals=fliterVals.split(',');
            result = result.filter(x=>!fliterVals.includes(x.id));
        }else if(axrFilter.toLowerCase().indexOf("in(")>-1)
        {
            let fliterVals=axrFilter.substr(3);
            fliterVals=fliterVals.substr(0,fliterVals.length-1);
            fliterVals=fliterVals.split(',');
            result = result.filter(x=>fliterVals.includes(x.id));
        }
