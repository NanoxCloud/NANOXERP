...
function CallAction(actionName, fileup, confirmmsg, remarks, manRemarks, dsignaction, isScript) {
    ...
    if (proceed) {
        try{
            ...
            if (actInd > -1 && typeof actScriptTask != "undefined" && (actScriptTask[actInd].startsWith("loadform♠") || actScriptTask[actInd].startsWith("loadformanddata♠") || actScriptTask[actInd].startsWith("loadiview♠") || actScriptTask[actInd].startsWith("loadpage♠") || actScriptTask[actInd].startsWith("openpage♠"))) {
...
function CallActionExt(actionName, fileup, remarks, dsignaction, isScript) {
    ...
    try {
        ...
        GetProcessTime();
        ASB.WebService.CallActionWS(ChangedFields, ChangedFieldDbRowNo, ChangedFieldValues, DeletedDCRows, DeletedFieldValue, ArrActionLog, visDcname, txt, fup, "t", delRows, chngRows, tstDataId, files, isScript, SuccessCallbackAction, OnException);
    }
    catch (exp) {
        actionCallbackFlag = actionCallFlag;
        $("#icons,#btnSaveTst,.BottomToolbarBar a,.wizardNextPrevWrapper").css({ "pointer-events": "auto" });
...
function DataReloadDuringActionError()
{    
    ...
    var CallAvActionCB = $.confirm({
        ...
        content: clickedButtonCaption +" not completed due to an unexpected error.",
        buttons: {
            buttonA: {
                text: "Retry",
                btnClass: 'hotbtn',
                        ...
                        if (comboField != undefined) {
                            ...
                            if (comboField.prop("type") == "select-one") {
                                ...
                                if (IsChkList) {
                                    ...
                                    fldClass = "type=radio class='form-check-input h-40px w-40px multiFldRdg'";
                                }
    ...
    for (var l = 0; l < parseInt(maxLevels); l++) {
        ...
        if (statusWrk[l] == '') {
            ...
            lblst.forEach(function (val) {
                if ((val.startsWith("Pending ") || val.startsWith(" Pending ")) && val.indexOf("'" + wfUser + "'") > 0)
                    htmlwfNew = "Approval pending with you. ";
                else if (val.startsWith("Returned ") && val.indexOf("'" + wfUser + "'") > 0)
