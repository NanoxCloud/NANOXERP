        ...
        if (typeof CKEDITOR.instances[$(ui).find("textarea").attr("id")] != "undefined") {
            ...
            try {
                ...
                if(!staticRunMode) {
                    ...
                    _this.resize('100%', $(_this.ui.editor.container.$).height() -  $(_this.ui.contentsElement.$).siblings().toArray().reduce((total, elm)=>{
...
function DropzoneInit(dvId) {
    ...
    if (typeof dvId != "undefined") {
        ...
        _thisDiv =$("[id^='divDc']:not(:has(.editWrapTr)) .dropzone");// $("#divDc1 .dropzone");
    }
...
function rtfCkeditorAlignment() {    
    ...
    for (var B = 0; B < A.length; B++) {
        if (A[B].id.startsWith("rtf_") || A[B].id.startsWith("rtfm_") || A[B].id.startsWith("fr_rtf_") || GetDWBFieldType(GetFieldsName(A[B].id)) == "Rich Text") {
            A[B].parentElement.classList.add("flex-root");
            A[B].parentElement.parentElement.classList.add("flex-column");
        }
    }        
}
...
// function callAxpertStudioAddForm() {
