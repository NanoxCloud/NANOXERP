...
public partial class Signin : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            requestProcess_logtime += ObjExecTr.WireElapsTime(browserElapstm, StartTime);
            isPostback = "true";
            panelSignin.Visible = false;
        }
    }
        ...
        try
        {
                    ...
                    if (!string.IsNullOrEmpty(code))
                    {
                        ...
                        if (tokenInfo != null)
                        {
                            if (!string.IsNullOrEmpty(tokenInfo.access_token))
                            {
                                ...
                                CallLoginService(true, profile.email, axPassword.Value, Session["Project"].ToString(), Session["AxLanguages"].ToString(), profile.id);// tokenInfo.access_token);
                            }
                        }
                    }
                }
            ...
            else if (provider == "facebook")
            {
                if (Request["code"] != null)
                {
                    ...
                    CallLoginService(true, objUser.email, axPassword.Value, Session["Project"].ToString(), Session["AxLanguages"].ToString(), tokenInfo.access_token);
                }
                Session.Remove("LoginWith");
            }
            else if (provider == "office365")
            {
                ...
                if (officeDetail != string.Empty)
                {
                    ...
                    CallLoginService(true, officeDetails[1].ToString(), axPassword.Value, Session["Project"].ToString(), Session["AxLanguages"].ToString(), officeDetails[0].ToString());
                }
                Session.Remove("LoginWith");
            }
            ...
            else if (provider == "saml")
            {
                try
                {
                    ...
                    if (samlDetail != string.Empty)
                    {
                        ...
                        CallLoginService(true, samlDetails[1].ToString(), axPassword.Value, Session["Project"].ToString(), Session["AxLanguages"].ToString(), samlDetails[0].ToString());
                    }
                }
                catch (Exception ex) { }
                Session.Remove("LoginWith");
    ...
    {
        //CallLoginService(false, axUserName.Value, axPassword.Value, axSelectProj.Value, axLanguage.Value);
        ...
        // UserIsLoggedIn(false, axUserName.Value, axPassword.Value, axSelectProj.Value, axLanguage.Value, staySignIn);
    }
    ...
    private void CallLoginService(bool isSSO, string userName, string password, string project, string language, string ssoKey = "")
    {
        ...
        else
        {
            ...
            strParams.Append("<input type=hidden name=\"hdnSSTime\" value=\"" + StartTime + "\">");
            ...
            try
            {
                ...
                if (true == adAuth.IsAuthenticated(dname[0], axUserName.Value, axPassword.Value))
                {
                    ...
                    if (Session["AxLanguages"] != null)
                        ...
                        thisLang = axLanguage.Value;
                    CallLoginService(true, axUserName.Value, "", thisProj, thisLang, SSOKey);
                }
                else
                {
                    string loginPath = Application["LoginPath"].ToString();
        ...
        try
        {
            ...
            if (hybridDetails != string.Empty)
            {
                ...
                if (savedUrl == urlDomain && prjName == projName)
                {
                    ...
                    axLanguage.Value = hybridDetails.Split('~')[4];
    ...
    protected string GetDiffTime()
    {
        ...
        try
        {
            ...
            diffTime = bst.Subtract(DateTime.Parse(StartTime)).TotalMilliseconds.ToString();
        }
        catch (Exception ex) { }
        return diffTime;
    }
    ...
    protected void SamlBtn_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (signedin.Checked)
                ...
                staySignIn = "false";
            Session["Project"] = axSelectProj.Value;
            Session["AxLanguages"] = axLanguage.Value;
    ...
    public static string KeepSigninWebDetailsNew(string hdnBwsrid)
    {
        ...
        try
        {
            ...
            var KeysList = fObj.GetAllKeys(rsKey);
            if (KeysList.Count > 0)
            {
        ...
        try
        {
            ...
            if (kaDetails != string.Empty)
            {
                ...
                if (savedUrl == urlDomain)
                    result = "{\"signedin\":\"true\",\"axSelectProj\":\"" + kaDetails.Split('~')[0] + "\",\"axUserName\":\"" + kaDetails.Split('~')[1] + "\",\"KeepMeAutoPwd\":\"" + kaDetails.Split('~')[2] + "\",\"axLanguage\":\"" + kaDetails.Split('~')[3] + "\",\"hdnLastOpenpage\":\"" + kaDetails.Split('~')[6] + "\"}";
            }
        }
        catch (Exception ex)
        {
